...
export function createClipboardHandler({
	...
	return async function handle(action, id, raw) {
		...
		busy = true;
		try {
      if (platform !== "darwin" || !["read", "write", "response"].includes(action))
				throw new Error();
			let text = "";
      if (action !== "read") {
        const limit = action === "response" ? 4_000_000 : 12000;
        if (typeof raw !== "string" || raw.length > limit * 6 + 100) throw new Error();
        text = JSON.parse(raw).text;
        if (typeof text !== "string" || text.length > limit) throw new Error();
			}
        ...
        child.stdin?.end(action !== "read" ? text : undefined);
			});
			if (
				action === "read" &&
			...
			reply(id, {
				ok: false,
        error: action === "response" ? "Clipboard unavailable or response exceeds 4 million characters." : "Clipboard unavailable or text exceeds 12,000 characters.",
			});
		} finally {
			busy = false;
		}
