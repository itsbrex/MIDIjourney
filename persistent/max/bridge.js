...
function validate(value) {
  if (!value || typeof value.title !== "string" || !value.title.trim() || value.title.length > 240 ||
      !finite(value.duration) || value.duration <= 0 || value.duration > 1024 ||
  ...
  return { title: value.title.replace(/[\r\n\u0000]/g, " "), duration: value.duration, notes: value.notes.map(function (n) {
    if (!finite(n.pitch) || n.pitch % 1 || n.pitch < 0 || n.pitch > 127 ||
      !finite(n.velocity) || n.velocity < 1 || n.velocity > 127 ||
...
function create(id, raw) {
  ...
  function finish(value) {
    ...
    if (!value.ok) post("MIDIjourney Persistent: " + value.error + "\n");
