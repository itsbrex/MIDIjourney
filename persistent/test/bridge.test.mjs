...
const { buildRequest } = createRequire(import.meta.url)("../../js/history.js");
const { CONFIG } = createRequire(import.meta.url)("../../js/config.js");

const source = await readFile(new URL("../max/bridge.js", import.meta.url), "utf8");
...
test("original Max input technique and persistent request have matching MIDI semantics", async () => {
  ...
  const inputPatch = JSON.parse(await readFile(new URL("../../patchers/mj_inputClipData.maxpat", import.meta.url))).patcher;
  const importPatch = JSON.parse(await readFile(new URL("../../patchers/mj_clipImport.maxpat", import.meta.url))).patcher;
