...
test("new device has stereo pass-through and an independent modeless web window", () => {
	const { patcher } = makeDevice(resolve("persistent/dist"));
	...
	assert.equal(window.title, "MIDIjourney Persistent");
...
test("Pollinations account sources stay identical to the preserved web version", async () => {
	// Chat rendering and copy-only response capture evolve; auth does not.
	for (const filename of ["main.tsx", "BrowserConnect.tsx", "live-auth.mjs"]) {
		const [persistent, beta] = await Promise.all([
			readFile(new URL(`../src/${filename}`, import.meta.url), "utf8"),
			readFile(new URL(`../../web/src/${filename}`, import.meta.url), "utf8"),
		]);
		// apply_patch's source snapshot may add a trailing blank line; source bodies
		// must otherwise remain identical, including all auth and package usage.
		assert.equal(persistent.trimEnd(), beta.trimEnd(), filename);
	}
});
