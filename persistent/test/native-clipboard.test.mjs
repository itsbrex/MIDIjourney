...
import { createClipboardHandler } from "../max/clipboard.mjs";
	...
	const host = {
		max: {
			...
			outlet(action, id, raw) {
				void handler(action === "clipboard_read" ? "read" : "write", id, raw);
			},
		},
	};
...
test("native responses are routed back to jweb, not the page navigation inlet", () => {
	...
	assert.equal(boxes.get("route").text, "route uiurl clipboard_response");
